...
public final class GameColor {
    ...
    public static GameColor fromRgbArray(Node node) {
        return new GameColor(
                ...
                node.getNodeArray().stream().map(Node::getString).collect(Collectors.toList()));
    }

    public static GameColor fromColorNode(Node n) {
        if (!n.isTagged()
                || Arrays.binarySearch(TaggedNode.COLORS, n.getTaggedNode().getType(), null) == -1) {
            throw new IllegalArgumentException("Invalid color node " + n.toString());
        }

        return new GameColor(
                n.getTaggedNode().getType(),
                n.getTaggedNode().getValues().stream().map(Node::getString).collect(Collectors.toList()));
    }
