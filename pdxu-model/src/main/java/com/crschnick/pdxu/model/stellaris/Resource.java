...
public class Resource {
    ...
    public static Optional<Resource> parseFromCountryNode(Node node, String name) {
        ...
        budget.get()
                ...
                .ifPresent(incomeNode -> incomeNode.forEach((k, v) -> {
        ...
        budget.get()
                .getNodeForKeyIfExistent("expenses")
                .ifPresent(expensesNode -> expensesNode.forEach((k, v) -> {
                    v.forEach((resourceName, amount) -> {
                        if (name.equals(resourceName)) {
        ...
        var stored = node.getNodeForKeysIfExistent("modules", "standard_economy_module", "resources", name)
                .map(Node::getDouble)
                .orElse(0.0);
        return Optional.ofNullable(Resource.builder()
                .stored(stored)
                .income(income[0])
                .expense(expense[0])
                .build());
