...
public class GuiEditorTypes {
    ...
    private static Region createCollectorTypeNode() {
        return createTypeNode(new Color(0.8, 0.3, 0.9, 0.8), 'S', new Insets(0, 0, 1, 1), "Synthetic collection");
    }
    ...
    private static Region createTypeNode(Node.Descriptor desc) {
        if (desc.getValueType() == null
            ...
            return createTypeNode(new Color(0.2, 0.2, 0.2, 0.3), 'C', new Insets(0, 2, 1, 0), "Complex type");
        }

        if (desc.getValueType() == Node.ValueType.BOOLEAN) {
            return createTypeNode(new Color(0.2, 0.2, 1, 0.5), 'B', new Insets(0, 0, 1, 1), "Boolean");
        }
        if (desc.getValueType() == Node.ValueType.TEXT) {
            return createTypeNode(new Color(0.2, 1, 0.2, 0.5), 'T', new Insets(1, 0, 0, 1), "Text");
        }
        if (desc.getValueType() == Node.ValueType.INTEGER) {
            return createTypeNode(new Color(1, 0.2, 0.2, 0.5), 'I', new Insets(0, 0, 1, 1), "Integer");
        }

        if (desc.getValueType() == Node.ValueType.FLOATING_POINT) {
            return createTypeNode(new Color(0.4, 1, 1, 0.5), 'F', new Insets(1, 0.3, 0, 1), "Floating point number");
        }

        if (desc.getValueType() == Node.ValueType.UNQUOTED_STRING) {
            return createTypeNode(new Color(1, 0.8, 0.3, 0.5), 'V', new Insets(1, 0.3, 0, 1), "Game specific value");
        }
        if (desc.getValueType() == Node.ValueType.COLOR) {
            return createTypeNode(new Color(0.9, 0.3, 0.9, 0.3), 'C', new Insets(0, 2, 1, 0), "Color");
        }
