...
public class Eu5SavegameAdapter implements EditorSavegameAdapter {
    ...
    public Map<String, NodePointer> createCommonJumps(EditorState state) {
        ...
        map.put("DLCs", NodePointer.builder().name("metadata").name("dlcs").build());
        map.put(
                "Settings",
    ...
    private NodePointer get(String key, String val) {
        ...
        if (RULER_TERM_KEYS.contains(key)) {
            return NodePointer.builder()
                    .name("rulerterm_manager")
                    .name("database")
                    .name(val)
                    .build();
        }

        if (MARKET_KEYS.contains(key)) {
            return NodePointer.builder()
                    .name("market_manager")
                    .name("database")
                    .name(val)
