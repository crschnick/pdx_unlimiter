...
public class Ck3SavegameAdapter implements EditorSavegameAdapter {
    ...
    private static final List<GuiEditorNodeTagFactory> FACTORIES = List.of(
            ...
            new ImagePreview(Path.of("gfx").resolve("coat_of_arms").resolve("patterns"), "pattern", "mdi-file"),
            new ImagePreview(Path.of("gfx").resolve("coat_of_arms").resolve("colored_emblems"), "texture", "mdi-file"),
    ...
    public Map<String, NodePointer> createCommonJumps(EditorState state) {
        var player = NodePointer.builder()
                .name("currently_played_characters")
                .index(0)
                .build();
        var playerDyn = NodePointer.builder()
                .name("living")
                .pointerEvaluation(player)
                .name("dynasty_house")
                .build();
        var houseDyn = NodePointer.builder()
                .name("dynasties")
                .name("dynasty_house")
                .pointerEvaluation(playerDyn)
                .build();
        var dynasty = NodePointer.builder()
                .name("dynasties")
                .name("dynasties")
                .pointerEvaluation(
                        NodePointer.fromBase(houseDyn).name("dynasty").build())
                .build();
        ...
        map.put(
                ...
                NodePointer.builder().name("currently_played_characters").build());
        map.put(
                "Player character",
                NodePointer.builder().name("living").pointerEvaluation(player).build());
        map.put(
                "Player realm",
                NodePointer.builder()
                        .name("living")
        ...
        var primaryTitle = NodePointer.builder()
                .name("landed_titles")
                .name("landed_titles")
                .pointerEvaluation(NodePointer.builder()
                        .name("living")
                        .pointerEvaluation(player)
                        .name("landed_data")
                        .name("domain")
        ...
        var primaryTitleCoaId =
                NodePointer.fromBase(primaryTitle).name("coat_of_arms_id").build();
        var primaryTitleCoa = NodePointer.builder()
                .name("coat_of_arms")
                .name("coat_of_arms_manager_database")
                .pointerEvaluation(primaryTitleCoaId)
                .build();
        map.put("Player primary title coat of arms", primaryTitleCoa);

        var houseCoaId = NodePointer.builder()
                .name("coat_of_arms")
                .name("coat_of_arms_manager_database")
                .function((root, n) -> {
                    var houseCoa = NodePointer.fromBase(houseDyn)
                            .name("coat_of_arms_id")
                            .build();
                    ...
                    var dynCoa = NodePointer.fromBase(dynasty)
                            .name("coat_of_arms_id")
                            .build();
                    val = dynCoa.get(root);
                    if (val != null) {
        ...
        var dynastyCoa = NodePointer.builder()
                .name("coat_of_arms")
                .name("coat_of_arms_manager_database")
                .function((root, n) -> {
                    var dynCoa = NodePointer.fromBase(dynasty)
    ...
    public List<NodePointer> createNodeJumps(EditorState state, EditorRealNode node) throws Exception {
        ...
        var parentKey = Optional.ofNullable(node.getParent()).flatMap(p -> p.getKeyName());

        var isDirectJump = keyOpt.isPresent()
                && CHARACTER_KEYS.contains(keyOpt.get())
                && node.getBackingNode().isValue();
        var isArrayJump = keyOpt.isEmpty()
                && parentKey.isPresent()
                && CHARACTER_KEYS.contains(parentKey.get())
                && node.getBackingNode().isValue();
        if (isDirectJump || isArrayJump) {
            var livingPointer = NodePointer.builder()
                    .name("living")
                    .name(node.getBackingNode().getString())
                    .build();
            var deadPointer = NodePointer.builder()
                    .name("dead_unprunable")
                    .name(node.getBackingNode().getString())
                    .build();
    ...
    public NodePointer createNodeJump(EditorState state, EditorRealNode node) {
        ...
        if (keyOpt.isPresent()
                && parentKey.isPresent()
                && node.getBackingNode().isValue()
                && keyOpt.get().equals("identity")) {
            var parent = node.getParent();
            boolean isTypeChar = parent.isReal()
                    && ((EditorRealNode) parent)
                            .getBackingNode()
                            .getNodeForKeyIfExistent("type")
                            .map(t -> t.isValue() && t.getString().equals("char"))
                            .orElse(false);
            if (isTypeChar) {
                return NodePointer.builder()
                        .name("living")
        ...
        if (keyOpt.isPresent()
                && node.getBackingNode().isArray()
                && node.getBackingNode().getArrayNode().size() == 1
                && node.getBackingNode().getNodeArray().getFirst().isValue()) {
            return get(
                    keyOpt.get(),
                    node.getBackingNode().getNodeArray().getFirst().getString());
        }
    ...
    public javafx.scene.Node createNodeTag(EditorState state, EditorRealNode node, Region valueDisplay) {
        return FACTORIES.stream()
                ...
                .findFirst()
                .map(fac -> fac.create(state, node, valueDisplay))
                .orElse(null);
    }
}
