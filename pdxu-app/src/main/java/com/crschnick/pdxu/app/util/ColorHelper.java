    ...
    public static Color fromGameColor(GameColor color) {
        ...
        try {
            ...
            return switch (color.getType()) {
                case HSV -> Color.hsb(d0 * 360, Math.min(d1, 1.0), Math.min(d2, 1.0));
                case HSV360 -> Color.hsb(d0, Math.min(d1 / 100.0, 1.0), Math.min(d2 / 100.0, 1.0));
                case RGB -> {
                    var isDecimal = c.get(0).contains(".")
                            || c.get(1).contains(".")
                            || c.get(2).contains(".");
                    var denominator = isDecimal ? 1.0 : 255.0;
                    yield Color.color(d0 / denominator, d1 / denominator, d2 / denominator);
                }
