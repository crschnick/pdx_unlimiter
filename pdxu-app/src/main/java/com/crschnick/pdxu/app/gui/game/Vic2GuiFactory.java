...
public class Vic2GuiFactory extends GameGuiFactory<Vic2Tag, Vic2SavegameInfo> {
    ...
    public Image tagImage(SavegameInfo<Vic2Tag> info, Vic2Tag tag) {
        return ImageHelper.loadImage(GameInstallation.ALL
                .get(Game.VIC2)
                .getInstallDir()
                .resolve("gfx")
                .resolve("flags")
                .resolve(tag.getTagId() + ".tga"));
    }

    @Override
    public Pane createIcon() {
        var icon = GameImage.getGameIcon(Game.VIC2);
        var cut = ImageHelper.cut(
                icon, new Rectangle2D(125, 0, Math.max(0, icon.getWidth() - 240), Math.max(0, icon.getHeight())));
